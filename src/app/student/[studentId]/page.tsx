...
import StudentSessions from "@/components/studentSessionSection";

export default async function CoachSelection({
  params,
}: {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {coaches.map(
          (coach: { id: number; name: string; time_zone: string }) => (
            <Link
              key={coach.id}
              href={`/student/${params.studentId}/${coach.id}`}
            >
      ...
      <StudentSessions studentId={params.studentId} />
    </div>
  );
}
